module.exports = function(options) {
  ...
  return `<picture class="${cssClass} ${className}">
    ...
    <img src="${root}assets/img/content/${name}--xs.${ext}" srcset="${root}assets/img/content/${name}--xs@2x.${ext} 2x, ${root}assets/img/content/${name}--xs@3x.${ext} 3x" ${width ? `width="${width}"` : ``} ${height ? `height="${height}"` : ``} ${alt ? `alt="${alt}"` : `alt=""`} />
  </picture>`
}
